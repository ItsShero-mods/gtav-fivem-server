...
-- Save a new note
RegisterNetEvent('qb-phone:server:SaveNote', function(note)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if note.id then
        ...
        MySQL.update(
            'UPDATE player_notes SET title = ?, content = ? WHERE id = ? AND citizenid = ?',
            { tostring(title), tostring(content), note.id, citizenid }
        )
    else
        -- New note: INSERT
        MySQL.insert(
            'INSERT INTO player_notes (citizenid, title, content) VALUES (?, ?, ?)',
            { citizenid, tostring(title), tostring(content) }
        )
    end
end)

-- Retrieve notes when the player opens the app
QBCore.Functions.CreateCallback('qb-phone:server:GetNotes', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return cb({}) end

    local citizenid = Player.PlayerData.citizenid
    local result = MySQL.query.await('SELECT id, title, content, created_at FROM player_notes WHERE citizenid = ?', { citizenid })
    cb(result or {})
end)

RegisterNetEvent('qb-phone:server:DeleteNote', function(noteId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    -- Make sure players can only delete *their* notes
    MySQL.execute('DELETE FROM player_notes WHERE id = ? AND citizenid = ?', {
        noteId,
        citizenid
    })
end)
